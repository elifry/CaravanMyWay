    ...
    public static class FloatMenuMakerMap_AddHumanlikeOrders_Patch
    {
        ...
        public static void Postfix(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
        {
            ...
            if (thing.stackCount > 1)
            {
                ...
                opts.Add(new FloatMenuOption($"Load specific amount into caravan...", delegate
                {
                ...
                // Add "Load all" option
                opts.Add(new FloatMenuOption($"Load all into caravan", delegate
                {
                    List<FloatMenuOption> carrierOptions = new List<FloatMenuOption>();
                    foreach (var carrier in lord.ownedPawns.Where(p => MassUtility.Capacity(p) > 0))
                    {
            ...
            else
            {
                ...
                opts.Add(new FloatMenuOption($"Load into caravan", delegate
                {
                    List<FloatMenuOption> carrierOptions = new List<FloatMenuOption>();
                    foreach (var carrier in lord.ownedPawns.Where(p => MassUtility.Capacity(p) > 0))
                    {
